...
static void process (menu_t *menu) {
    bool tabs_blocked = pane_focused
        && panes[selected_pane]->blocks_tabs
        && panes[selected_pane]->blocks_tabs(menu);

    if (!tabs_blocked && (menu->actions.tab_left || menu->actions.tab_right)) {
        if (pane_focused) {
            leave_pane(menu);
        }
