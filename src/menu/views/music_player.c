...
static void format_elapsed_duration (char *buffer, float elapsed, float duration) {
    ...
    if (duration >= 3600) {
        sprintf(buffer + strlen(buffer), "%02u:", (uint16_t) (elapsed) / 3600);
    }
    sprintf(buffer + strlen(buffer), "%02u:%02u", ((uint16_t) (elapsed) % 3600) / 60, (uint16_t) (elapsed) % 60);

    strcat(buffer, " / ");

    if (duration >= 3600) {
        sprintf(buffer + strlen(buffer), "%02u:", (uint16_t) (duration) / 3600);
    }
    sprintf(buffer + strlen(buffer), "%02u:%02u", ((uint16_t) (duration) % 3600) / 60, (uint16_t) (duration) % 60);
}
