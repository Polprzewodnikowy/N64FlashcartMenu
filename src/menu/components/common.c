...
void gui_component_layout_draw (void) {
    gui_component_border_draw(
        VISIBLE_AREA_X0,
        VISIBLE_AREA_Y0,
        VISIBLE_AREA_X1,
        VISIBLE_AREA_Y1
    );
    gui_component_box_draw(
        VISIBLE_AREA_X0,
...
void gui_component_progressbar_draw (int x0, int y0, int x1, int y1, float progress) {    
    ...
    gui_component_box_draw(x0, y0, x0 + progress_width, y1, PROGRESSBAR_DONE_COLOR);
    gui_component_box_draw(x0 + progress_width, y0, x1, y1, PROGRESSBAR_BG_COLOR);
}

void gui_component_seekbar_draw (float position) {
    int x0 = SEEKBAR_X;
    int y0 = SEEKBAR_Y;
    int x1 = SEEKBAR_X + SEEKBAR_WIDTH;
    int y1 = SEEKBAR_Y + SEEKBAR_HEIGHT;

    gui_component_border_draw(x0, y0, x1, y1);
    gui_component_progressbar_draw(x0, y0, x1, y1, position);
}

void gui_component_loader_draw (float progress) {
    int x0 = LOADER_X;
    int y0 = LOADER_Y;
    ...
    gui_component_border_draw(x0, y0, x1, y1);
    gui_component_progressbar_draw(x0, y0, x1, y1, progress);
}
...
void gui_component_scrollbar_draw (int x, int y, int width, int height, int position, int items, int visible_items) {
    if (items <= 1 || items <= visible_items) {
        gui_component_box_draw(x, y, x + width, y + height, SCROLLBAR_INACTIVE_COLOR);
    } else {
        int scroll_height = (int) ((visible_items / (float) (items)) * height);
        float scroll_position = ((position / (float) (items - 1)) * (height - scroll_height));

        gui_component_box_draw(x, y, x + width, y + height, SCROLLBAR_BG_COLOR);
        gui_component_box_draw(x, y + scroll_position, x + width, y + scroll_position + scroll_height, SCROLLBAR_POSITION_COLOR);
    }
}

void gui_component_list_scrollbar_draw (int position, int items, int visible_items) {
    gui_component_scrollbar_draw(
        LIST_SCROLLBAR_X,
        LIST_SCROLLBAR_Y,
...
void gui_component_dialog_draw (int width, int height) {
    ...
    gui_component_border_draw(x0, y0, x1, y1);
    gui_component_box_draw(x0, y0, x1, y1, DIALOG_BG_COLOR);
}
