...
static const char *format_rom_save_type (rom_save_type_t save_type, bool supports_cpak) {
    switch (save_type) {
        ...
        case SAVE_TYPE_FLASHRAM_FAKE: return supports_cpak ? "FlashRAM (Wrongly implemented) | Controller PAK" : "FlashRAM (Wrongly implemented)";
        default: return "Unknown";
    }
}
...
static component_context_menu_t set_save_type_context_menu = { .list = {
    ...
    { .text = "FlashRAM Non-compliant", .action = set_save_type, .arg = (void *) (SAVE_TYPE_FLASHRAM_FAKE) },
    COMPONENT_CONTEXT_MENU_LIST_END,
}};
