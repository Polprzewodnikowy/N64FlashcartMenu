...
static void process (menu_t *menu) {
    if (gui_component_context_menu_process(menu, &entry_context_menu)) {
        return;
    }

    if (gui_component_context_menu_process(menu, &settings_context_menu)) {
        return;
    }
    ...
    if (menu->actions.enter && menu->browser.entry) {
        ...
        gui_component_context_menu_show(&entry_context_menu);
        sound_play_effect(SFX_SETTING);
    } else if (menu->actions.settings) {
        gui_component_context_menu_show(&settings_context_menu);
        sound_play_effect(SFX_SETTING);
    }
}
...
static void draw (menu_t *menu, surface_t *d) {
    ...
    gui_component_file_list_draw(menu->browser.list, menu->browser.entries, menu->browser.selected);
    ...
    gui_component_context_menu_draw(&entry_context_menu);

    gui_component_context_menu_draw(&settings_context_menu);
...
void view_browser_init (menu_t *menu) {
    if (!menu->browser.valid) {
        gui_component_context_menu_init(&entry_context_menu);
        gui_component_context_menu_init(&settings_context_menu);
        if (load_directory(menu)) {
            path_free(menu->browser.directory);
