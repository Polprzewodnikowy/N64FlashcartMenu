...
void ui_component_progressbar_draw (int x0, int y0, int x1, int y1, float progress) {    
    ...
    ui_component_box_draw(x0, y0, x0 + progress_width, y1, PROGRESSBAR_DONE_COLOR);
    ui_component_box_draw(x0 + progress_width, y0, x1, y1, PROGRESSBAR_BG_COLOR);
}

void ui_component_seekbar_draw (float position) {
    int x0 = SEEKBAR_X;
    int y0 = SEEKBAR_Y;
    int x1 = SEEKBAR_X + SEEKBAR_WIDTH;
    int y1 = SEEKBAR_Y + SEEKBAR_HEIGHT;

    ui_component_border_draw(x0, y0, x1, y1);
    ui_component_progressbar_draw(x0, y0, x1, y1, position);
}

void ui_component_loader_draw (float progress) {
    int x0 = LOADER_X;
    int y0 = LOADER_Y;
    ...
    ui_component_border_draw(x0, y0, x1, y1);
    ui_component_progressbar_draw(x0, y0, x1, y1, progress);
}
...
void ui_component_scrollbar_draw (int x, int y, int width, int height, int position, int items, int visible_items) {
    if (items <= 1 || items <= visible_items) {
        ui_component_box_draw(x, y, x + width, y + height, SCROLLBAR_INACTIVE_COLOR);
    } else {
        int scroll_height = (int) ((visible_items / (float) (items)) * height);
        float scroll_position = ((position / (float) (items - 1)) * (height - scroll_height));

        ui_component_box_draw(x, y, x + width, y + height, SCROLLBAR_BG_COLOR);
        ui_component_box_draw(x, y + scroll_position, x + width, y + scroll_position + scroll_height, SCROLLBAR_POSITION_COLOR);
    }
}

void ui_component_list_scrollbar_draw (int position, int items, int visible_items) {
    ui_component_scrollbar_draw(
        LIST_SCROLLBAR_X,
        LIST_SCROLLBAR_Y,
...
void ui_component_dialog_draw (int width, int height) {
    ...
    ui_component_border_draw(x0, y0, x1, y1);
    ui_component_box_draw(x0, y0, x1, y1, DIALOG_BG_COLOR);
}
