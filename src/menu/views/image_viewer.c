...
static void draw (menu_t *menu, surface_t *d) {
    if (!image) {
        ...
        gui_component_loader_draw(png_decoder_get_progress());
    } else {
        ...
        if (show_message) {
            gui_component_messagebox_draw(
                "Set \"%s\" as background image?\n\n"
                "A: Yes, B: Back",
                menu->browser.entry->name
            );
        } else if (image_set_as_background) {
            gui_component_messagebox_draw("Preparing background…");
        }
    }
...
static void deinit (menu_t *menu) {
    ...
    if (image) {
        if (image_set_as_background) {
            gui_component_background_replace_image(image);
        } else {
            surface_free(image);
            free(image);
        }
